<script lang="ts">
	...
	// if (access_token) {
	// 	const response = await fetch(`http://localhost:3000/WebsocketGateway/getClient`, {
	// 	method: 'GET',
	// 	headers: {
	...
	// 	});
            const onlineUsers_url = "http://${LOCALHOST}:3000/auth/onlineUsers";
                    const response = await fetch(onlineUsers_url, {
				method: "GET",
				headers: {
					Authorization: `Bearer ${access_token}`,
					"Content-Type": "application/json",
    ...
    if (access_token) {
        ...
        const response = await fetch(`http://${LOCALHOST}:3000/user/profile`, { headers });
    ...
    async function fetchRoomGameId() {
		...
		if (accessToken) {
			...
			const response = await fetch(
				`http://${LOCALHOST}:3000/user/getRoomId`,
				{
					headers
				}
			);
 ...
 //   }
    ...
    try {
		    ...
		    if (accessToken) {
                ...
                console.log("&dskjfqdqjsfljqskfldqs----------------------");
                const data =  await response.json();
        ...
        <div class="dm_body">
            ...
            <div class="right_bloc">
                ...
                <div class="selctedUser_button_settings">
                    ...
                    <p> <div role="button" tabindex="0" class="button_show_profile" 
                        on:click={handleCheckProfileButton} 
                        on:keydown={handleCheckProfileButton}>
                        Profile
                      </div>
